...
const FIXED_RATES    = { BYND: 1 }
    , BYND_MSAT_RATIO = big('100000000000')
...
const getRateCoingecko = currency =>
  request.get(`https://api.coingecko.com/api/v3/simple/price?ids=beyondcoin&vs_currencies=${enc(currency)}`)
    .proxy(RATE_PROXY)
    .then(res => res.body.beyondcoin[currency.toLowerCase()] || Promise.reject(`Unknown currency: ${currency}`))
